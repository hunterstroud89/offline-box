...
function toggleSidebar() {
    ...
    if (sidebar) {
        ...
        localStorage.setItem('ollama-sidebar-collapsed', isCollapsed.toString());
    }
}
...
function initializeMobileSidebar() {
    ...
    // Load user's sidebar preference
    const savedCollapsed = localStorage.getItem('ollama-sidebar-collapsed');
    
    function updateSidebarForScreenSize() {
        if (window.innerWidth <= 768) {
            // Mobile: ensure sidebar starts collapsed if not already set
            if (!sidebar.classList.contains('collapsed')) {
                sidebar.classList.add('collapsed');
                console.log('Mobile detected: Sidebar collapsed');
            }
            ...
            // Desktop: restore user's preference or default to expanded
            if (savedCollapsed === 'true') {
                sidebar.classList.add('collapsed');
                console.log('Desktop detected: Restored collapsed state');
            } else if (savedCollapsed === 'false') {
                sidebar.classList.remove('collapsed');
                console.log('Desktop detected: Restored expanded state');
            }
            // If no preference saved, leave current state as is
            
            if (sidebarToggle) {
                // Always show toggle button on desktop so users can toggle
                sidebarToggle.style.display = 'flex';
                console.log('Desktop detected: Toggle button available');
            }
        }
    }
...
async function sendMessage() {
    ...
    try {
        ...
        if (data.error) {
            ...
            addMessage(errorMessage, 'assistant', false, true);
